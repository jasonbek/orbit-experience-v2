...
export function Trajectory() {
    ...
    const brandColor = isMissionControl ? "#009DD6" : "#D80010"; // Hex for SVG
    const variant = isMissionControl ? "blue" : "red";           // Variant for Glass
    ...
    return (
        // 2. The Frame: Padding creates the "Floating Dashboard" effect
        <div className="absolute inset-0 z-0 pointer-events-auto flex items-center justify-center p-2 md:p-4 lg:p-6 transition-all duration-700">

            {/* 3. The Glass Dashboard: Merged Container */}
            <GlassContainer
                variant={variant}
                className="relative w-full h-full overflow-hidden shadow-2xl border-white/10"
            >
                {/* --- LAYER A: PLANETS (The Anchors) --- */}
                {/* Earth (Launch) */}
                <motion.div
                    initial={{ opacity: 0, scale: 0.8 }}
                    animate={{ opacity: 0.8, scale: 1 }}
                    transition={{ duration: 1.5 }}
                    className="absolute bottom-[-10%] left-[-5%] w-[40%] h-[40%] md:w-[25%] md:h-[25%] opacity-20 pointer-events-none z-0"
                >
                ...
                <motion.div
                    ...
                    className="absolute top-[-5%] right-[-5%] w-[30%] h-[30%] md:w-[20%] md:h-[20%] opacity-20 pointer-events-none z-0"
                >
                    <div className="relative w-full h-full grayscale opacity-40 mix-blend-screen">
